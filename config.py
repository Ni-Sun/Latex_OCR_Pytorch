...
# batch_size = 1 #训练解批大小, 本地运行时使用1的批大小，避免显存不足
batch_size = 8 #训练解批大小, kaggle平台使用P100有16GB显存，建议使用8的批大小以加快训练速度
test_batch_size = 16 #验证集批大小, 验证集无需反向传播，可以使用更大的批大小
encoder_lr = 1e-4  # 学习率
decoder_lr = 4e-4  # 学习率
grad_clip = 5.  # 梯度裁剪阈值
alpha_c = 1.  # regularization parameter for 'doubly stochastic attention', as in the paper
best_score = 0.  # 目前最好的 score 
print_freq = 50  # 状态的批次打印间隔, 减少日志IO开销
